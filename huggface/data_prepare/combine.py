...
tokenized_datasets = hklii_dataset.map(
    tokenize_function, batched=True, remove_columns=["ID","topic","paragraphs"],num_proc = 32)
print(tokenized_datasets)

tokenized_datasets = tokenized_datasets.filter(lambda example: len(example['input_ids']) > 7,num_proc=32)
print(tokenized_datasets)
...
tokenized_datasets.save_to_disk("/home/huijie/legal/huggface/data_prepare/HKLII_all")
