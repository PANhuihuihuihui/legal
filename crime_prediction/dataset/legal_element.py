...
def get_element_datasets(args):
    tokenizer = BertTokenizer.from_pretrained(args.back_bone)

    def tokenize_function(examples):
        result = tokenizer(examples["sentence"],truncation = True,max_length = args.sentence_len)
        if tokenizer.is_fast:
            result["word_ids"] = [result.word_ids(i) for i in range(len(result["input_ids"]))]
        return result

    dataset = load_dataset("json", data_files="/home/huijie/legal/mymodel/data/legal_element.json")
    dataset = dataset.shuffle(seed=args.seed)["train"].rename_column("labels","classification").train_test_split(0.1)
    tokenized_datasets = dataset.map(
        tokenize_function, batched=True, remove_columns=["class","sentence"],num_proc =8
    )
...
def get_scm_datasets(args):
    ...
    dataset = load_dataset("json", data_files="/home/huijie/legal/mymodel/data/legal_element.json")
    dataset = dataset.shuffle(seed=args.seed)["train"].rename_column("lables","classification").train_test_split(0.1)
    tokenized_datasets = dataset.map(
