...
flags.DEFINE_string(
    "domain", "divorce",
    "The name of the task to train selected in the list: divorce, labor, loan")

flags.DEFINE_string(
    "model_type", "rcnnatt",
    "The name of the layer to train after bert selected in the list: " + ", ".join(modelMap.keys()))
###bert后接的模型类型，如rcnn，如果为None则直接全连接层

flags.DEFINE_string(
    "data_dir", os.path.join('data', FLAGS.domain),
...
flags.DEFINE_string(
    "bert_config_file", os.path.join(FLAGS.bert_path, 'bert_config.json'),
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "multilabelclassfier", "The name of the task to train.")

flags.DEFINE_string("vocab_file", os.path.join(FLAGS.bert_path, 'vocab.txt'),
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", 'ckpt/' + FLAGS.domain,
    "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", os.path.join(FLAGS.bert_path, 'bert_model.ckpt'),
    "Initial checkpoint (usually from a pre-trained BERT model).")

# flags.DEFINE_string(
#     "init_checkpoint", os.path.join(pretrain, task, 'bert_model.ckpt'),
#     "Initial checkpoint (usually from a pre-trained BERT model).")
...
flags.DEFINE_integer(
    "max_seq_length", domain2len[FLAGS.domain],
...
flags.DEFINE_integer(
    ...
    "The number of keep_checkpoint_max. "
    )
...
class MultilabelClassfier(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        ###整理测试文件，记得换成自己合并一二阶段的数据集
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "train_selected.json")), "train")
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    label_id = [0] * len(label_map)  ##在此处就将标签转化为one hot
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    if FLAGS.model_type:
        ...
        model_layer = modelMap[FLAGS.model_type](
            embedding=embedding, context_dim=200, hidden_dim=200, dropout_keep_prob=0.9
        )
    ...
    loss, per_example_loss, probabilities = focal_loss(logits, labels)
    return (loss, per_example_loss, logits, probabilities)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        ####create_new将数据分别使用两种loss计算，create_model只使用focal loss。
...
def main(_):
    ...
    processors = {
        "multilabelclassfier": MultilabelClassfier,
    }
    ...
    if FLAGS.do_train:
        ...
        save_checkpoints_steps = int(num_train_steps/FLAGS.num_train_epochs * 2 + 0.5)
        if FLAGS.save_checkpoints_epochs:
            save_checkpoints_steps = int(num_train_steps / FLAGS.num_train_epochs * FLAGS.save_checkpoints_epochs) + 1
